...
DEEPSEEK_API_KEY = os.getenv("DEEPSEEK_API_KEY")  # Get API key from environment variable
...
if not DEEPSEEK_API_KEY:
    logger.error("DEEPSEEK_API_KEY environment variable is not set. The application will not function correctly.")
...
async def health_check():
    try:
        ...
        return JSONResponse(
            status_code=500,
            content={"status": "unhealthy", "error": str(e), "api_key_set": bool(DEEPSEEK_API_KEY)}
        )
